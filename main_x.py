...
# MongoDB Connection
client = MongoClient("mongodb+srv://shravya7321:lVMFwzt9AcGBwXhg@cluster0.hblvkf7.mongodb.net/")
db = client["gym_app"]
...
def save_to_db(email, exercise_name, reps):
    global user_email
    if email and exercise_name and reps >= 0:
        record = {
            "email": email,
        ...
        user_data_collection.insert_one(record)


def process_webcam(exercise_type, flip_camera=True, audio_feedback=True):
    global user_email
    global engine, exercise_reps, exercise_state, feedback_message, feedback_color, exercise_started
    
    # Convert exercise name to type number
    exercise_type = list(exercise_map.values()).index(exercise_type) + 1
    ...
    try:
        ...
        return f"Workout saved! {exercise_name}: {exercise_reps} reps"

# # Water tracker logic
# def update_water_intake(amount):
#     global water_intake
#     water_intake += amount
#     return f"Total water intake: {water_intake} ml / {water_goal} ml"

# def reset_water_tracker():
#     global water_intake
#     water_intake = 0
#     return f"Water tracker reset. Current intake: {water_intake} ml"

# def calculate_next_period(last_period_start, cycle_length):
#     try:
#         last_period_start_date = datetime.datetime.strptime(last_period_start, "%Y-%m-%d")
#         next_period_start_date = last_period_start_date + datetime.timedelta(days=cycle_length)
#         return next_period_start_date.strftime("%Y-%m-%d")
#     except ValueError:
#         return "Invalid Date Format"
    
# # Function to track the period status
# def period_status(last_period_start, cycle_length):
#     try:
#         last_period_start_date = datetime.datetime.strptime(last_period_start, "%Y-%m-%d")
#         today = datetime.datetime.today()
#         next_period_start_date = last_period_start_date + datetime.timedelta(days=cycle_length)
        
#         # Check if the current date is within the period window (Assuming period lasts 7 days)
#         period_end_date = next_period_start_date + datetime.timedelta(days=7)
        
#         if last_period_start_date <= today <= period_end_date:
#             status = "You are currently on your period."
#         elif today > period_end_date:
#             status = "You are not on your period right now."
#         elif today >= next_period_start_date - datetime.timedelta(days=3):
#             status = "Your period is approaching."
#         else:
#             status = "Your period is not due yet."
        
#         return f"Next period starts on: {next_period_start_date.strftime('%Y-%m-%d')}\n{status}"
#     except ValueError:
#         return "Invalid Date Format"            
...
def chatbot_interface():
    with gr.Row():
        ...
        restrictions = gr.Checkbox(label="Any injuries or limitations?")

    chatbot_output = gr.Textbox(label="Bot Response", lines=5)
    chatbot_input = gr.Textbox(label="Ask me about workouts or fitness")
    send_button = gr.Button("Send")
    listen_button = gr.Button("🎤 Speak")

    def on_click(user_input, goal, experience, restrictions):
        return process_chatbot_input(user_input, goal, experience, restrictions)

    def on_speak(goal, experience, restrictions):
        query = listen()
        return process_chatbot_input(query, goal, experience, restrictions)

    send_button.click(on_click, inputs=[chatbot_input, goal, experience, restrictions], outputs=chatbot_output)
    listen_button.click(on_speak, inputs=[goal, experience, restrictions], outputs=chatbot_output)

    return gr.Column([goal, experience, restrictions, chatbot_input, send_button, listen_button, chatbot_output])
    ...
    with gr.Blocks(title="FlexiFit") as app:
        ...
        # Exercise Tracker Tab
            with gr.Tab("Exercise Tracker"):
                ...
                process_button.click(process_webcam, inputs=[exercise_dropdown, flip_camera, audio_feedback], outputs=[output_text])
            
            # Water Tracker Tab
            with gr.Tab("Water Tracker"):
                build_water_tracker_ui()
                # gr.Markdown("Track your daily water intake.")
                
                # with gr.Row():
                #     water_input = gr.Number(label="Enter water intake (ml)", value=250)
                #     water_update_btn = gr.Button("Add Intake")
                #     water_reset_btn = gr.Button("Reset Tracker")
                    
                #     water_output = gr.Textbox(label="Water Intake Status")
                #     water_update_btn.click(update_water_intake, inputs=water_input, outputs=water_output)
                #     water_reset_btn.click(reset_water_tracker, inputs=None, outputs=water_output)
            
            # Period Tracker Tab
            with gr.Tab("Period Tracker"):
                period_tracker_tab()
                # gr.Markdown("Track your menstrual cycle and the status of your next period.")
                
                # with gr.Row():
                #     with gr.Column(scale=2):
                #         # User inputs for Period Tracker
                #         last_period_input = gr.Textbox(label="Enter the start date of your last period (YYYY-MM-DD)", placeholder="2025-03-20")
                #         cycle_length_input = gr.Number(label="Enter your average cycle length (in days)", value=28)
                    
                #     with gr.Column(scale=3):
                #         # Outputs for period status
                #         next_period_output = gr.Textbox(label="Next Period Start Date")
                #         period_status_output = gr.Textbox(label="Period Status")
                    
                # # Button to calculate and update period status
                # calculate_period_btn = gr.Button("Calculate Next Period")
                # calculate_period_btn.click(
                #     calculate_next_period,
                #     inputs=[last_period_input, cycle_length_input],
                #     outputs=next_period_output
                # )
                
                # calculate_period_btn.click(
                #     period_status,
                #     inputs=[last_period_input, cycle_length_input],
                #     outputs=period_status_output
                # )
            ...
            login_button.click(authenticate, inputs=[email_input], outputs=[main_ui, error_message])
        return app

# # Add the period tracker to your Gradio interface
# def gradio_exercise_tracker():
#     with gr.Blocks(title="FlexiFit") as app:
#         gr.Markdown("# FlexiFit")
...
#             return f"Workout saved! {exercise_name}: {exercise_reps} reps"
#         else:
#             return f"Error saving workout data. Please try again."
